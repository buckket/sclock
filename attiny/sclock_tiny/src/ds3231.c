...
void DS3231_enable_alarm2(void) {
	...
	TWI0_write(0xB); // Set pointer to Alarm2 Register
...
uint8_t DS3231_read_control(void) {
	TWI0_start(DS3231_ADDRESS_WRITE);
	TWI0_write(0xE); // Set pointer to Control Register
	TWI0_stop();
...
uint8_t DS3231_read_status(void) {
	TWI0_start(DS3231_ADDRESS_WRITE);
	TWI0_write(0xF); // Set pointer to Status Register
	TWI0_stop();
...
void DS3231_read_time(time_struct *clock) {
	TWI0_start(DS3231_ADDRESS_WRITE);
	TWI0_write(0x0); // Set pointer to zero
	TWI0_stop();
...
void DS3231_read_alarm1(time_struct *clock) {
	TWI0_start(DS3231_ADDRESS_WRITE);
	TWI0_write(0x7); // Set pointer to ALARM1
	TWI0_stop();
